    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            if (completion >= 0.9 && !hasEnteredIntersection)
            {
                ...
                auto this_shared = get_shared_this();
                std::future<void> ftr = std::async([this_shared](std::shared_ptr<Intersection> intersection){
                  intersection->addVehicleToQueue(this_shared);
                }, _currDestination);
                // std::future<void> ftr = std::async(_currDestination->addVehicleToQueue, get_shared_this());
                ftr.wait();
